...
$logres = login_cmd("event CUSTOM sofia::register sofia::unregister$BLANK");
$eventcount = 0;
%Channel_Spool = ();
local $| = 1;
...
while (<$remote>) {
	...
	if ($_ eq "") {
		if ($finalline =~ /Event\-Name/) {
			...
			elsif ($event{'Event-Subclass'} eq "sofia%3A%3Aregister") {&do_register(%event);}
			elsif ($event{'Event-Subclass'} eq "sofia%3A%3Aunregister") {&do_unregister(%event);}
...
sub do_register() {
	...
	local $contact =  uri_unescape($event{'contact'});
	...
	$ext = $event{'from-user'};
	$host = $event{'from-host'};
	
	my $memcache = "";
    my $memcache = new Cache::Memcached {'servers' => ['127.0.0.1:11211'],};
	
	$memcache->set($ext . '@' . $host, $dt);
	warn "[$time]Save " . $ext . '@' . $host . " ==> " . $dt . "!\n";
}
sub do_unregister() {
	local(%event) = @_;
