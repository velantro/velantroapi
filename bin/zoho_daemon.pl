...
sub End() {
	...
	$data = "type=$type&state=$state&id=$uuid&from=$from&to=$to&start_time=$starttime&duration=$billsec&voiceuri=https://$domain_name/app/xml_cdr/download.php?id=$uuid&t=bin" . #uri_escape('https://$domain_name/app/xml_cdr/download.php?id=$uuid&t=bin');
