...
if ($query{action} eq 'addwidget') {
	...
	get_agent_status();
} elsif ($query{action} eq 'setagentstatus'){
	set_agent_status();
} else {
...
sub set_recording {
	...
	my $result = `fs_cli -x 'db insert/vconnect_record/$exten/$enable'`;
...
sub get_register {
	...
	for (split /\n/, `sudo /usr/local/bin/fs_cli -x "show registrations"`) {
...
=pod	
	my $cnt	= int (`fs_cli -x "db select/livecalls/$extension"`);
	$cnt  ||= 0;
...
sub check_did_available {
	...
	my $cnt	= int (`fs_cli -x "db select/channels/$did"`);
	$cnt  ||= 0;
...
sub send_callback {	
	...
	if (1 or $domain eq 'rapidins.velantro.net'){
		$subdir = `date +'%Y/%b/%d'`;
		chomp $subdir;
		$recording_path = "/usr/local/freeswitch/recordings/rapidins.velantro.net/archive/$subdir/$uuid.wav";
		$cmd = qq{$fs_cli -x "bgapi originate {ringback=/var/www/api/usring.wav,ignore_early_media=true,absolute_codec_string=PCMA,fromextension=$ext,origination_caller_id_name=$dest,origination_caller_id_number=$dest,effective_caller_id_number=$dest,effective_caller_id_name=$dest,domain_name=$domain,outbound_caller_id_number=$dest,$alert_info,origination_uuid=$uuid,RECORD_STEREO=true,$auto_answer,execute_on_originate='record_session::$recording_path'}user/$ext\@$domain &bridge([origination_caller_id_name=$cid,origination_caller_id_number=$cid,effective_caller_id_number=$cid,effective_caller_id_name=$cid,iscallback=$ext,outbound_caller_id_number=$cid,sip_h_X-accountcode=1234323300]sofia/gateway/8544913a-168c-458a-83a7-33febf41e8de/$dest)"};
		#$cmd = qq{$fs_cli -x "bgapi originate {ringback=/var/www/api/usring.wav,ignore_early_media=true,absolute_codec_string=PCMA,fromextension=$ext,origination_caller_id_name=$dest,origination_caller_id_number=$dest,effective_caller_id_number=$dest,effective_caller_id_name=$dest,domain_name=$domain,outbound_caller_id_number=$dest,$alert_info,origination_uuid=$uuid,$auto_answer}loopback/$ext/$domain &bridge([origination_caller_id_name=$cid,origination_caller_id_number=$cid,effective_caller_id_number=$cid,effective_caller_id_name=$cid,iscallback=$ext,outbound_caller_id_number=$cid,sip_h_X-accountcode=8706018290]sofia/gateway/69cf2c01-974b-4661-92cc-cd732488ba2f/$dest)"};
	}
	warn "$cmd\n";
	my $result = `$cmd`;
...
sub get_vconnectincoming {
	...
	for my $line (split /\n/, $channels) {
		...
		my $fromextension = `fs_cli -x "uuid_getvar $uuid fromextension"`;
		chomp $ext;
...
sub get_autocallstate {
	...
	for my $line (split /\n/, $channels) {
		...
		my $is_lead = `fs_cli -x "uuid_getvar $uuid is_lead"`;
...
sub set_agent_status {
	...
	if (!$agent_uuid) {
		...
		my $result = `fs_cli -x "callcenter_config agent set status $agent_uuid '$status'"`;
		chomp $result;
		print j({error => '0', 'message' => $result});
	}
