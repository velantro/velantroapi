...
sub send_callback {	
	...
	if ($domain eq 'rapidins.velantro.net'){
		$cmd = qq{$fs_cli -x "bgapi originate {ringback=/var/www/api/usring.wav,ignore_early_media=true,absolute_codec_string=PCMU,fromextension=$ext,origination_caller_id_name=$dest,origination_caller_id_number=$dest,effective_caller_id_number=$dest,effective_caller_id_name=$dest,domain_name=$domain,outbound_caller_id_number=$dest,$alert_info,origination_uuid=$uuid,RECORD_STEREO=true,$auto_answer,execute_on_originate='record_session::/tmp/$uuid.wav'}user/$ext\@$domain &bridge([origination_caller_id_name=$cid,origination_caller_id_number=$cid,effective_caller_id_number=$cid,effective_caller_id_name=$cid,iscallback=$ext,outbound_caller_id_number=$cid,sip_h_X-accountcode=8706018290]sofia/gateway/69cf2c01-974b-4661-92cc-cd732488ba2f/$dest)"};
