...
//get http post variables and set them to php variables
	if (is_array($_POST)) {
		//get the post variables a run a security chack on them
			...
			$queue_callback = $_POST["queue_callback"];
...
//process the user data and save it to the database
	if (count($_POST) > 0 && strlen($_POST["persistformvar"]) == 0) {
		...
		//prepare the array
			...
			$array['call_center_queues'][0]['queue_callback'] = $queue_callback;
		...
		//build the xml dialplan
			...
			if ($queue_callback == 'true') {
				#$dialplan_xml .= "		<action application=\"bind_digit_action\" data=\"queue-callback,*,exec:execute_extension,callback-\${caller_id_number}-\${destination_number}1 XML \${domain_name}\"/>\n";
				$dialplan_xml .= "		<action application=\"bind_digit_action\" data=\"queue-callback,*,exec:lua,queue_callback.lua \${caller_id_number} \${destination_number} \${uuid} \${domain_name}\"/>\n";
			}
	...
	if (is_array($_GET) && is_uuid($_GET["id"]) && $_POST["persistformvar"] != "true") {
		...
		if (is_array($call_center_queues)) {
			foreach ($call_center_queues as &$row) {
				...
				$queue_callback = $row["queue_callback"];
...
//show the content
	...
	echo "</td>\n";
	echo "<td class='vtable' align='left'>\n";
	echo "	<select class='formfld' name='queue_callback'>\n";
	if ($queue_callback == "false") {
		echo "	<option value='false' selected='selected' >".$text['option-false']."</option>\n";
	}
	else {
		echo "	<option value='false'>".$text['option-false']."</option>\n";
	...
	echo "	</select>\n";
	echo "<br />\n";
	echo "Enable / Disable Queue Callback"."\n";
	echo "</td>\n";
	echo "</tr>\n";
