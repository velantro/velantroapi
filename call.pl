...
sub send_callback {	
	...
	if ($dest =~ /^011/) {
		...
		&send_zoho_request('clicktodialerror', $ext.'@'.$domain, "type=dialed&state=invalid&id=$uuid&from=$ext&to=$dest&start_time=$starttime&message=International Dialing is disabled");
		exit 0;
	}
	...
	unless($dest =~ /^\d{10}$/) {
		...
		&send_zoho_request('clicktodialerror', $ext.'@'.$domain, "type=dialed&state=invalid&id=$uuid&from=$ext&to=$dest&start_time=$starttime");
		exit 0;
	}
	...
	#system("mkdir -p /var/lib/freeswitch/recordings/c2c/");
	my $recording_filename = "/var/lib/freeswitch/recordings/$domain/archive/". strftime('%Y', localtime()) . "/" . strftime('%b',  localtime()) . "/" . strftime('%d', localtime()) .  "/$uuid.wav";
	my $cmd = qq{$fs_cli -x "bgapi originate {ignore_early_media=true,absolute_codec_string=PCMA,fromextension=$ext,origination_caller_id_name=$dest,origination_caller_id_number=$dest,effective_caller_id_number=$dest,effective_caller_id_name=$dest,domain_name=$domain,outbound_caller_id_number=$dest,$alert_info,origination_uuid=$uuid,api_on_answer='uuid_record $uuid start $recording_filename',$auto_answer}user/$ext\@$domain &bridge([origination_caller_id_name=$cid,origination_caller_id_number=$cid,effective_caller_id_number=$cid,effective_caller_id_name=$cid,iscallback=$ext,outbound_caller_id_number=$cid,user_record=all,record_session=true,ring_ready=true,ignore_early_media=true]loopback/$dest/$domain)"};
