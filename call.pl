...
sub send_callback {	
	...
	if ($dest =~ /^011/) {
		...
		&send_zoho_request('clicktodialerror', $ext.'@'.$domain, "code=blocked&from=$ext&to=$dest&message=International Dialing is disabled");
		exit 0;
	}
	...
	unless($dest =~ /^\d{10}$/) {
		...
		&send_zoho_request('clicktodialerror', $ext.'@'.$domain, "code=invalid&from=$ext&to=$dest&message=invalid number");
		exit 0;
	}
